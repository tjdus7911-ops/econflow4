  ...
  return `
    <article class="issue-card tone-${issue.tone} ${index === 0 ? "is-featured" : ""}">
      ...
      <div class="issue-actions">
        <button class="text-button" data-node="${issue.nodeId}">WHY 왜?</button>
        <button class="primary-button small" data-node="${issue.nodeId}">IMPACT 영향 보기 ${icon("arrow", 15)}</button>
      </div>
...
function renderNodeDetail(nodeId) {
  ...
  return `<aside class="node-detail-panel">
    ...
    ${selected ? `<div class="selected-edge-card"><div class="selected-edge-top"><span>선택한 연결</span><button data-action="edge-clear">${icon("close", 14)}</button></div><strong>${getNode(selected.from).name} ${icon("arrow", 14)} ${getNode(selected.to).name}</strong><p>${selected.description}</p><div class="evidence-mini">${icon("source", 14)} 경제적 전달 경로 설명</div></div>` : ""}

    <div class="panel-scroll">
      <div class="what-block"><span class="framework-label what">WHAT</span><h3>무슨 일이 있나요?</h3><p>${node.what}</p><div class="data-snapshot"><span>${node.metric}</span><strong>${node.change}</strong><small>Mock · ${node.updated}</small></div></div>
      ...
      <div class="detail-group evidence-group"><div class="detail-title-row"><h3>근거와 출처</h3><span>공식 우선</span></div>${node.sources.map((source) => `<div class="evidence-row">${icon("source", 16)}<span><strong>${source}</strong><small>공식 데이터 · 기준일 ${APP_DATE}</small></span><b>원문</b></div>`).join("")}<p class="evidence-note">이 설명은 인과를 확정하거나 미래 결과를 예측하지 않으며, 관련될 수 있는 경제적 경로를 보여줍니다.</p></div>
    </div>
  </aside>`;
}
...
function renderMarket() {
  ...
  return `
    <main class="page content-width standard-page">
      ...
      <section class="market-summary-bar"><div><span>시장 업데이트</span><strong>09월 23일 · 10:20</strong></div><div><span>주요 흐름</span><strong>금리 기대 · 환율 · AI</strong></div><div><span>데이터 기준</span><strong>15분 지연 Mock</strong></div><p>${icon("info", 15)} 실제 투자 판단용 데이터가 아니에요.</p></section>
      <section class="section-block market-section">
        ...
        ${sectionHeading("MARKET DATA", label, '<span class="section-note">변화의 좋고 나쁨이 아닌 연결 경로를 확인해 보세요</span>')}
        <div class="market-grid">${items.map((item) => {
          ...
          return `<article class="market-card"><div class="market-card-head">${regionMark(region)}<span>${region}</span><button class="mini-heart ${state.interests.includes(target) ? "is-active" : ""}" data-action="interest-toggle" data-id="${target}">${icon("heart", 16, state.interests.includes(target))}</button></div><h3>${name}</h3><div class="market-value"><strong>${value}</strong><span class="move-${direction}">${change}</span></div><div class="market-status"><span>현재 맥락</span><b>${status}</b></div><div class="sparkline ${direction}"><span></span><span></span><span></span><span></span><span></span><span></span><span></span></div><div class="market-actions"><button class="text-button" data-node="${target}">왜 움직였지?</button><button class="primary-button ghost small" data-node="${target}">관련 경제 흐름 ${icon("arrow", 14)}</button></div><div class="updated-line">${icon("clock", 13)} 10:20 업데이트 · Mock</div></article>`;
        }).join("")}</div>
      </section>
...
function eventCard(event) {
  ...
  return `<article class="event-card importance-${event.importance}">
    ...
    <div class="event-body"><div class="event-meta">${regionMark(event.region)}<span>${event.region}</span><i>${event.type}</i><b class="status-${event.status.replaceAll(" ", "-")}">${event.status}</b></div><h3>${event.title}</h3>${flowTrail(event.flow, true)}<p>${event.why}</p><div class="event-source">${icon("source", 14)} ${event.source}</div></div>
    <div class="event-actions"><button class="secondary-button" data-event="${event.id}">왜 중요한가?</button><button class="text-button" data-node="${event.nodeId}">경제 흐름 보기 ${icon("arrow", 14)}</button><button class="watch-icon ${watched ? "is-watched" : ""}" data-action="watch-event" data-id="${event.id}" title="일정 지켜보기">${icon(watched ? "check" : "bell", 17)}</button></div>
  </article>`;
}
...
function renderCalendar() {
  ...
  return `
    <main class="page content-width standard-page calendar-page">
      ...
      <section class="calendar-content">${state.calendarView === "month" ? renderMonthCalendar(events) : `<div class="upcoming-layout"><div class="date-rail"><span>SEP<strong>23</strong>오늘</span><i></i><span>OCT<strong>01</strong>8일 후</span><i></i><span>OCT<strong>30</strong>37일 후</span></div><div class="event-list">${events.map(eventCard).join("")}</div><aside class="calendar-aside"><div class="watch-summary"><span>${icon("bell", 20)}</span><h3>지켜보는 일정</h3><strong>${state.watchItems.filter((id) => economicEvents.some((event) => event.id === id)).length}개</strong><p>알림 기능은 mock 상태예요.<br>관심 일정은 MY에도 저장됩니다.</p><a href="#/my">MY에서 보기 ${icon("arrow", 14)}</a></div><div class="calendar-legend"><h4>일정 상태</h4><span><i class="confirmed"></i>확정</span><span><i class="planned"></i>예정</span><span><i class="tbd"></i>세부 일정 미정</span></div></aside></div>`}</section>
    </main>`;
}
...
function renderCompany(id = "samsung") {
  ...
  return `<main class="page content-width standard-page company-page"><a href="#/market" class="page-back">${icon("back", 16)} 시장으로 돌아가기</a><section class="company-header"><div class="company-identity"><span class="company-logo large">S</span><div><p>${company.code}</p><h1>${company.name}</h1><span>${company.description}</span></div></div><div class="company-price"><span>현재가 · Mock</span><strong>${company.price}</strong><b>${company.change}</b><small>${company.updated}</small></div><div class="company-header-actions"><button class="icon-action ${state.interests.includes("samsung") ? "is-active" : ""}" data-action="interest-toggle" data-id="samsung">${icon("heart", 18, state.interests.includes("samsung"))}</button><button class="primary-button" data-node="samsung">경제지도에서 보기 ${icon("arrow", 15)}</button></div></section><section class="company-main-grid"><div class="chart-card"><div class="card-title-row"><div><span class="section-kicker">PRICE CONTEXT</span><h2>가격 흐름</h2></div><div class="range-tabs"><button>1일</button><button>1주</button><button class="is-active">3개월</button><button>1년</button></div></div>${chartSvg(company.chart)}<div class="chart-axis"><span>06.23</span><span>07.23</span><span>08.23</span><span>09.23</span></div></div><aside class="company-flow-card"><span class="section-kicker">CONNECTED FLOW</span><h2>현재 연결된<br>경제 흐름</h2>${company.flows.map((flow) => `<button data-node="${flow.target}"><span>${flow.label}</span>${flowTrail(flow.nodes, true)}<b>흐름 보기 ${icon("arrow", 14)}</b></button>`).join("")}</aside></section><section class="company-detail-section"><div class="tab-row">${[["overview", "기업정보"], ["financials", "재무·실적"], ["disclosures", "공시"], ["news", "관련 뉴스"]].map(([tab, label]) => `<button class="tab-button ${state.companyTab === tab ? "is-active" : ""}" data-action="company-tab" data-id="${tab}">${label}</button>`).join("")}</div>${companyTabContent(company)}</section><div class="data-disclaimer">${icon("info", 15)} 이 페이지의 가격·재무 수치는 UI 테스트용 mock data이며, 투자 추천이나 매수·매도 판단을 제공하지 않습니다.</div></main>`;
}
...
function renderEventModal() {
  ...
  if (!event) return "";
  const watched = state.watchItems.includes(event.id);
  return `<div class="modal-backdrop event-backdrop" data-action="event-close"><aside class="event-modal" role="dialog" aria-modal="true" data-modal-stop><button class="modal-close" data-action="event-close">${icon("close", 19)}</button><div class="event-modal-top">${regionMark(event.region)}<span>${event.region} · ${event.type}</span><b>${event.status}</b></div><p class="section-kicker">${event.dayLabel} · ${event.time}</p><h2>${event.title}</h2><div class="event-why"><span class="framework-label why">WHY</span><h3>왜 중요한가요?</h3><p>${event.why}</p></div><div class="event-modal-flow"><span class="framework-label impact">IMPACT</span><h3>관련 경제 흐름</h3>${flowTrail(event.flow)}</div><div class="event-watch-list"><span>앞으로 함께 확인할 것</span><ul><li>정책 결정 또는 지표 발표 결과</li><li>이전 전망과 달라진 설명</li><li>연결된 금리·환율 데이터의 반응</li></ul></div><div class="event-modal-source">${icon("source", 16)}<span><strong>${event.source}</strong><small>공식 일정 · ${APP_DATE} 확인 기준</small></span></div><div class="event-modal-actions"><button class="secondary-button ${watched ? "is-watched" : ""}" data-action="watch-event" data-id="${event.id}">${icon(watched ? "check" : "bell", 16)} ${watched ? "지켜보는 중" : "일정 지켜보기"}</button><button class="primary-button" data-node="${event.nodeId}">경제 흐름 보기 ${icon("arrow", 15)}</button></div></aside></div>`;
}
...
function render() {
  ...
  const validPages = ["today", "explore", "market", "calendar", "my", "company"];
  ...
  app.innerHTML = `${renderHeader(page)}${content}${page === "explore" ? "" : renderFooter()}${renderSearchModal()}${renderEventModal()}${state.toast ? `<div class="toast">${icon("check", 16)} ${state.toast}</div>` : ""}`;
  document.body.classList.toggle("has-modal", state.searchOpen || Boolean(state.modalEvent));
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Escape") {
    if (state.searchOpen || state.modalEvent) {
      state.searchOpen = false;
      state.modalEvent = null;
      render();
    }
